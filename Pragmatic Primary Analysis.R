...
# Binary electability variable for Top 4 candidates (1: Most electable, 0: Not) (WAVE 4)
...
dat$strat_4<-NA
for(x in 1:nrow(dat)){
  if(is.na(dat$vote.electable_4[x]) | is.na(dat$vote.therm_4[x]) | is.na(dat$vote.viable_4[x]) | 
     is.na(dat$therm.tie_4[x]))
  {dat$strat_4[x]<-NA}
  else if((dat$vote.electable_4[x]==1 | dat$vote.viable_4[x]==1) & 
          (dat$vote.therm_4[x]!=1 | (dat$vote.therm_4[x]==1 & dat$therm.tie_4[x]>1)))
  {dat$strat_4[x]<-1}
...
for(x in 1:nrow(dat)){
  if(is.na(dat$vote.electable_5[x]) | is.na(dat$vote.therm_5[x]) | is.na(dat$vote.viable_5[x]) | 
     is.na(dat$therm.tie_5[x]))
  {dat$strat_5[x]<-NA}
  else if((dat$vote.electable_5[x]==1 | dat$vote.viable_5[x]==1) & 
          (dat$vote.therm_5[x]!=1 | (dat$vote.therm_5[x]==1 & dat$therm.tie_5[x]>1)))
  {dat$strat_5[x]<-1}
...
# Change in strategic voting from wave 4 to wave 5

dat <- dat %>%
  mutate(strat.change=strat_5-strat_4)
